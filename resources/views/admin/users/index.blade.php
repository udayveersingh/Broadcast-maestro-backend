<x-layout.default>
                                        ...
                                        <form @submit.prevent="saveUser">
                                            ...
                                            <div class="mb-5">
                                                   <label for="ctnFile">file input</label>
                                                    <input id="ctnFile" type="file" class="form-input file:py-2 file:px-4 file:border-0 file:font-semibold p-0 file:bg-primary/90 ltr:file:mr-5 rtl:file:ml-5 file:text-white file:hover:bg-primary" x-model="params.avtar" />
                                            </div>
    ...
    async fetchContacts() {
        this.loading = true;
        try {
            const response = await fetch("/admin/get-users"); // your Laravel API route
            const data = await response.json();
            this.contactList = data.users.data || data;// assuming JSON returns { users: [...] }
            this.searchContacts();
        } catch (error) {
    ...
    async saveUser() {
        const headers = {
         ...
         if (!this.params.name || !this.params.email || !this.params.role || !this.params.status) {
            this.showMessage("All fields are required.", "error");
            return;
        }

        const isUpdate = !!this.params.id;
        const url = isUpdate ? `/admin/users/${this.params.id}` : `/admin/users`;
