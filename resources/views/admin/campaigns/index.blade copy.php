        ...
        document.addEventListener("alpine:init", () => {
            Alpine.data('invoiceList', () => ({
                ...
                async fetchCampaigns() {
                    try {
                        const response = await fetch('http://127.0.0.1:8000/admin/get-campaigns');
                        const result = await response.json();
                        this.items = result.data.map((campaign, index) => ({
                            id: campaign.id,
                            invoice: campaign.id.toString().padStart(6, '0'), // fake invoice ID
                            name: campaign.user?.name || 'Unknown',
                            ...
                            action: campaign.id,
                            avatar: `/storage/${campaign.user?.avatar}` || '/default-avatar.png', // adjust path if needed
                        }));
                    } catch (error) {
